...
#include <WiFi.h>                       // handles WiFi connectivity
#include <WebServer.h>                  // handles the web server creation
#include <ArduinoJson.h>                // handles the JSON data (for sending to app)
#include "Adafruit_VEML7700.h"          // handles the library for the sensors

// define the analog input pins for each soil moisture sensor
// ADC channels on the ESP32
#define AOUT_PIN 35                     // corresponds to sensor 1
#define AOUT_PIN2 34                    // corresponds to sensor 2
#define AOUT_PIN3 32                    // corresponds to sensor 3
...
// WiFi Credentials
const char* ssid = "Millers304";        // network name
const char* password = "Tland1971";     // network password

// hosting the web server on port 80
WebServer server(80);

// global var.s to track soil moisture of each environment
int lastMoistureValue1 = 0;             // corresponds to sensor 1
int lastMoistureValue2 = 0;             // corresponds to sensor 2
int lastMoistureValue3 = 0;             // corresponds to sensor 3

// tracks wet or dry tagging for the soil moisture of each environment
String lastStatus1 = "unknown";         // corresponds to sensor 1
String lastStatus2 = "unknown";         // corresponds to sensor 2
String lastStatus3 = "unknown";         // corresponds to sensor 3

// variables to track the timing of sensor readings
unsigned long lastReadTime1 = 0;        // corresponds to sensor 1
unsigned long lastReadTime2 = 0;        // corresponds to sensor 2
unsigned long lastReadTime3 = 0;        // corresponds to sensor 3

// the update rate for the sensor readings
const unsigned long interval = 1000;    // 1 second

// all of the one-time setup steps
void setup() {

  // initialize serial communication - baud rate 115,200 bps
  Serial.begin(115200);

  // intitalize the wifi library's network settings & returns the current status
  // returns: WL_CONNECTED (connected) or WL_IDLE_STATUS (not connected)
  WiFi.begin(ssid, password);

  Serial.print("Connecting to WiFi...");

  // print dots while the WiFi connection is loading
  // stay in this state until connected
  ...
  // creating an endpoint for the sensor readings on the ESP32's IP address
  // ex.  1.1.1.1/sensors
  // listens for GET requests at /sensors
  server.on("/sensors", HTTP_GET, []() {
    // creating a JSON document to hold the data
    StaticJsonDocument<200> doc;

    // add moisture values to JSON
    doc["value1"] = lastMoistureValue1;
    doc["value2"] = lastMoistureValue2;
    doc["value3"] = lastMoistureValue3;

    // add status strings to JSON
    doc["status1"] = lastStatus1;
    doc["status2"] = lastStatus2;
    doc["status3"] = lastStatus3;

    // will be used to convert JSON to string
    String json;
    ...
    // send response with JSON data to the server
...
void loop() {
  ...
  // handle any incoming client requests (this is where the updates happen)
  server.handleClient();

  // get the current time to track time since last update of value
  unsigned long now = millis();

  // if it's the proper time to read the sensors (1 second)
  if (now - lastReadTime1 >= interval) {

    // update the last read time to current time, for tracking next update
    lastReadTime1 = now;
...
void readSoilMoisture() {
  // read the moisture sensor values
    lastMoistureValue1 = analogRead(AOUT_PIN);
    lastMoistureValue2 = analogRead(AOUT_PIN2);
    lastMoistureValue3 = analogRead(AOUT_PIN3);

    // determine the moisture value "tag" as wet/dry based on the value read
    // for now, 2200 is the threshold
